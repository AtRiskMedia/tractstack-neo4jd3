...
export default function Neo4jD3(selector, _options) {
  ...
  function linkArc(d) {
    const r = Math.hypot(d.target.x - d.source.x, d.target.y - d.source.y) / d.linknum * 2.5;
    return `
  ...
  function init(selector, _options) {
    ...
    if (options && options?.neo4jData) {
      ...
      const simulation = d3.forceSimulation(nodes).force("charge", d3.forceManyBody().strength(options.strength)).force("link", d3.forceLink(relationships).id(function (d) {
        ...
        return (options?.distance | 100) + (options?.distance | 100) * 0.25 * Math.random();
      ...
      svg.selectAll(".relationship").append("text").attr("class", "text").attr("fill", "#000").attr("font-size", options.labelFontSize).attr("class", "text").append("textPath").attr("text-anchor", "middle").attr("startOffset", "50%").attr("xlink:xlink:href", function (d, i) {
        return "#edgepath" + i;
      }).text(function (d) {
        return d.type;
      });
      const allNodes = svg.append("g").attr("class", "nodes").attr("fill", "currentColor").attr("class", "node").attr("stroke-linecap", "round").attr("stroke-linejoin", "round").attr("cursor", "pointer").selectAll("g").data(nodes, function (d) {
        return d.id;
      }).join("g").attr("class", "node").selectAll("text").data(nodes).enter().append("text").attr("dx", 12).attr("dy", ".35em").text(function (d) {
        return typeof d?.properties?.object_name === "string" ? d.properties.object_name : d?.labels && d?.labels[0] && typeof d?.labels[0] === "string" ? d.labels[0] : "*";
      }).call(drag(simulation));
      allNodes.append("circle").attr("stroke", "#000").attr("stroke-width", 1).attr("fill", d => class2color(d.labels[0])).attr("r", 7).append("title").text(function (d) {
        return toString(d);
      });
